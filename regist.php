...
<?php 
   if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $stmt = $conn->prepare("SELECT * FROM accounts WHERE R_name = ?");
    $stmt->bind_param("s", $name);
    $stmt->execute();
    $stmt->store_result();
    if($stmt->num_rows >= 1){
      echo "Username \"$name\" is already existed! Change username please!";
    }else{
    ...
    $stmt = $conn->prepare("INSERT INTO accounts (R_name, R_pass) VALUES (?,?)");
